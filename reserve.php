...
<body>
    ...
    <div class="head">
        <h2></br> Reserve Books </h2>
    </div>

    <table class="form2">
        <tr>
            <td width="15%">
                <img src="right.png" width="15px">   <a class="selected3" href="reserve.php"> Reserve </a></br></br>
                <img src="right.png" width="15px">   <a class="nav2" href="return.php"> Return </a>
            </td>
            <td>
                <div class="div2">
                <p class="comment"> * please input full title and full author name - open Browse Books for details </p>
                <form action="reserve.php" class="login" method="post">
                    ...
                    <input type="submit" name="ressubmit" class="reserve-button" value="Reserve">
                </form>
                </div>
            </td>
        </tr>
    ...
    <?php
        ...
        if(isset($_POST['title']))    {
            ...
            $query = "SELECT `title` , `author` , `imageData` , `availability` 
                FROM `books` WHERE title='$title' AND author='$author'";
                ...
                while($row = mysqli_fetch_assoc($result)){ 
                    ...
                    if($row['availability']==1) {
                        ...
                        if($queryResults > 0)   {
                            while($row = mysqli_fetch_assoc($result))   {
                                if($row['book1']==NULL) {
                                    $query    = "UPDATE `member` SET `book1`='$title' WHERE username = '$un'";
                                    $result   = mysqli_query($con, $query) or die(mysqli_error($con));

                                    if ($result) {
                                        $sql = "UPDATE `books` SET `availability`='0' WHERE title='$title' AND author='$author'";
                                        if(mysqli_query($con, $sql))    {
                                            echo '<p class="default">';
                                            echo 'Reserved successfully!</p>';
                                            echo "</div></div>";
                                        }
                                        else {
                                ...
                                else if($row['book2']==NULL) {
                                    $query    = "UPDATE `member` SET `book2`='$title' WHERE username = '$un'";
                                    $result   = mysqli_query($con, $query) or die(mysqli_error($con));
                                ...
                                else if($row['book3']==NULL) {
                                    $query    = "UPDATE `member` SET `book3`='$title' WHERE username = '$un'";
                                    $result   = mysqli_query($con, $query) or die(mysqli_error($con));
                                ...
                                else if($row['book1']!=NULL && $row['book2']!=NULL && $row['book3']!=NULL) {
                                    echo '<p class="default">';
                                    echo 'You have reached the maximum reservation count. Please return a book to reserve again.';
                                    echo "</div></div>";
                                }
                            }
                        } 
                        ...
                        echo '<div class="form">';
                        echo '<div class="container2">';
                        echo '<p class="default">Book not available!</p>';
                        echo '</div></div>';
                    }
                }
            }
